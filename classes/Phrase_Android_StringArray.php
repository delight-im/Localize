...
class Phrase_Android_StringArray extends Phrase_Android {
    ...
    public function outputAndroidXML() {
        $valueEntries = array();
        foreach ($this->values as $value) {
            $valueEntries[] = "\t\t".'<item>'.self::writeToRaw($value, File_IO::FORMAT_ANDROID_XML).'</item>';
        }

        return "\t".'<string-array name="'.$this->phraseKey.'">' ."\n" . implode("\n", $valueEntries) . "\n"."\t".'</string-array>';
    }
    ...
    public function outputAndroidXMLEscapedHTML() {
        $valueEntries = array();
        foreach ($this->values as $value) {
            $valueEntries[] = "\t\t".'<item>'.self::writeToRaw($value, File_IO::FORMAT_ANDROID_XML_ESCAPED_HTML).'</item>';
        }
    ...
    public function outputJSON() {
        $valueEntries = array();
        foreach ($this->values as $value) {
            $valueEntries[] = "\t\t".'"'.self::writeToRaw($value, File_IO::FORMAT_JSON).'"';
        }

        return "\t".'"'.$this->phraseKey.'" : { "type" : "string-array", "content" : ['. "\n" . implode(",\n", $valueEntries) . "\n"."\t".'] }';
    }
    ...
    public function outputPlaintext() {
        $valueEntries = array();
        foreach ($this->values as $value) {
            $valueEntries[] = $this->phraseKey.';string-array;;'.self::writeToRaw($value, File_IO::FORMAT_PLAINTEXT);
        }
